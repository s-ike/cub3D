...
/*   Updated: 2021/02/14 11:45:08 by sikeda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_floor_color(t_info *info, t_floorcalc *calc, int y, int x)
{
	...
	tx = (int)fmin((int)(info->texture[TEX_FLOOR].w * (calc->floor_x - cell_x)),
		(info->texture[TEX_FLOOR].w - 1));
	ty = (int)fmin((int)(info->texture[TEX_FLOOR].h * (calc->floor_y - cell_y)),
...
static void	set_ceilling_color(t_info *info, t_floorcalc *calc, int y, int x)
{
	...
	cell_x = (int)calc->floor_x;
	cell_y = (int)calc->floor_y;
	tx = (int)fmin((int)(info->texture[TEX_CEILLING].w
		* (calc->floor_x - cell_x)), (info->texture[TEX_CEILLING].w - 1));
	ty = (int)fmin((int)(info->texture[TEX_CEILLING].h
		* (calc->floor_y - cell_y)), (info->texture[TEX_CEILLING].h - 1));
